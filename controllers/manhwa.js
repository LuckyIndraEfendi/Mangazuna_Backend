...
export const getAllManhwa = async (req, res) => {
  try {
    ...
    let hasNext = true;
    const prev = $(
      "#content > div.postbody > div > section > div.post > div.pagination > a.prev.page-numbers"
    )
      .text()
      .replace("«", "");
    $(
...
export const getmanhwaDetails = async (req, res) => {
  try {
    const response = await gotScraping.get(
...
export const getManhwaChapter = async (req, res) => {
  try {
    const response = await gotScraping.get(
